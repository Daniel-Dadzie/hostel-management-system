...
| status       | ENUM(PENDING,APPROVED,REJECTED) | Booking status   |
...
flowchart TD
    ...
    U --> W[Create Booking - CONFIRMED]
    V --> W
  
    W --> X[Payment Module ACTIVATED]
    X --> Y[Send Confirmation Email]
    Y --> Z[Student Proceeds to Payment]
  
    L --> AA[End Process - No Payment]
    M --> AA
```
...
| ----------------------------- | -------------- | ----------------- | ------------------ |
| Room Available                | CONFIRMED      | YES               | Confirmation Email |
...
5. **Conditional Payment**: Payment module is ONLY activated when room is successfully allocated
6. **Rejection Handling**: If no room available, booking is rejected WITHOUT triggering payment

### Booking Status Flow

| Status    | Description                 | Payment Status    | Next Action               |
| --------- | --------------------------- | ----------------- | ------------------------- |
| CONFIRMED | Room allocated successfully | PENDING - Created | Student makes payment     |
...
3. **Immediate Confirmation**: When room is available, booking is CONFIRMED immediately
4. **Payment Deadline**: Payment must be made within specified timeframe after confirmation
5. **Auto-Cancellation**: Unpaid confirmed bookings are auto-cancelled after deadline
...
public class RoomAllocationServiceImpl implements RoomAllocationService {
    ...
    public AllocationResult allocateRoom(Student student, RoomPreferenceRequest preferences) {
        ...
        // Step 6: Allocate room and update occupancy
        ...
        // Step 7: Create CONFIRMED booking - Payment WILL be triggered
...
public class BookingServiceImpl implements BookingService {
    ...
    public BookingResponse applyForHostel(Long studentId, RoomPreferenceRequest request) {
        ...
        // Room allocated successfully - payment is activated
        return BookingResponse.builder()
...
public class AllocationResult {
    ...
    public static AllocationResult confirmed(Booking booking, Payment payment) {
...
│              │  ├──────────────────────────────────────────┤│
│  Bookings    │  │John   │HostelA│101 │PENDING │Feb │[Approve][Reject]│
│              │  │Jane   │HostelB│205 │APPROVED│Feb │[View]           │
│  Payments    │  │Mike   │HostelA│102 │PENDING │Feb │[Approve][Reject]│
...
| POST   | `/api/student/apply`       | Apply for hostel     |
| POST   | `/api/student/preferences` | Set room preferences |
...
sequenceDiagram
    ...
    B->>D: Create booking request
    D-->>B: Booking created - PENDING
    B-->>F: Booking details
    F-->>S: Show application status

    S->>F: Set Preferences
    F->>B: POST /api/student/preferences
    B->>B: Run allocation algorithm
    B->>D: Find matching room
    D-->>B: Room found
    B->>D: Allocate room
    B-->>F: Preferences saved
    F-->>S: Show allocated room
```
...
sequenceDiagram
    ...
    A->>F: View Pending Bookings
    F->>B: GET /api/admin/bookings?status=PENDING
    B->>D: Query pending bookings
    D-->>B: Booking list
    B-->>F: Bookings data
    F-->>A: Display bookings

    A->>F: Approve Booking
    F->>B: PUT /api/admin/bookings/id/approve
    B->>D: Update booking status
    B->>D: Update room occupancy
    D-->>B: Success
    B-->>F: Updated booking
...
CREATE TABLE IF NOT EXISTS bookings (
    ...
    status ENUM('PENDING', 'CONFIRMED', 'REJECTED', 'CANCELLED') DEFAULT 'CONFIRMED',
    booking_date DATETIME DEFAULT CURRENT_TIMESTAMP,
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    FOREIGN KEY (student_id) REFERENCES students(id) ON DELETE CASCADE,
    FOREIGN KEY (room_id) REFERENCES rooms(id) ON DELETE CASCADE,
    UNIQUE KEY unique_student_booking (student_id)
);
...
| Pending Bookings  | Still pending                  |
...
| Export Reports            | PDF/Excel export for booking reports      | Planned         |
